...
class NETS(nn.Module):
    ...
    '''
    def __init__(self, encoder_name, input_size, learnSize, coord = True, pretrained = False, dev='cuda', cov=0.003):
    ...
    def forward(self, x):
        ...
        return {"logit" : logit, "hm" : hm, 'centres' : p, "embeded" : z}
    ...
    def initialize_Scale(self, dataLoader):
        
        self.DSNT_Scale = torch.zeros_like(self.DSNT_Scale, device='cpu')
        for imgs,labels,names in dataLoader:
            cropped_labels = cropFFT(labels, self.learnSize)
            cropped_labels = cropped_labels.reshape(-1,cropped_labels.shape[-1])
            self.DSNT_Scale = torch.maximum(cropped_labels.abs().max(dim=0)[0], self.DSNT_Scale)

        self.DSNT_Scale = 1.2*self.DSNT_Scale
        self.DSNT_Scale = self.DSNT_Scale.to(self.dev)

def cropFFT(y, learnSize):
    W, O = learnSize // 2, learnSize % 2
    y_low = torch.cat((y[...,0:W+O], y[...,-W:]), dim = -1)
    return y_low
